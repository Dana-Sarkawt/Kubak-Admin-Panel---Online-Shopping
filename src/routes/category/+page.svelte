<script lang="ts">
  ...
  let ArrayNumber: number[] = [1, 2, 3, 4, 5, 6, 7, 8];
  $: {
    pages.forEach((page) => {
...
<div class="w-full h-auto flex justify-center items-center mt-4">
  ...
  <div
    class="md:w-16 md:h-16 lg:w-24 lg:h-24 hover:scale-105 transition-ease-in-out duration-300 bg-[#f17f18] px-5 flex justify-center items-center flex-col rounded-xl mb-3"
  >
...
<div
  class="w-full h-auto flex flex-row-reverse flex-wrap justify-center items-center md:px-12 lg:px-26 xl:px-26 2xl:px-32 gap-3"
>
  {#each ArrayNumber as array}
    <div
      class="md:w-28 lg:w-36 md:h-36 lg:h-52 xl:w-60 xl:h-64 2xl:w-80 2xl:h-96 bg-white px-5 flex justify-around items-center flex-col rounded-xl"
    >
